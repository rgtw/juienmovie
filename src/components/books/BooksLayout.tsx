...
const tabs = [
  { href: '/books', label: '發現', icon: Library },
  { href: '/books/search', label: '搜索', icon: Search },
  { href: '/books/shelf', label: '書架', icon: BookOpen },
  { href: '/books/history', label: '歷史', icon: History },
];
...
function getStaticMeta(pathname: string) {
  if (pathname === '/books/shelf') return { title: '電子書書架', subtitle: '集中管理收藏的電子書' };
  if (pathname === '/books/history') return { title: '閱讀歷史', subtitle: '從上次閱讀的位置繼續' };
  if (pathname === '/books/search') return { title: '電子書搜索', subtitle: '按書名與作者搜索' };
  if (pathname === '/books/detail') return { title: '電子書詳情', subtitle: '查看書籍信息與可用格式' };
  if (pathname === '/books/read') return { title: '電子書閱讀', subtitle: '分頁閱讀', backHref: '/books' };
  return { title: '電子書館', subtitle: 'OPDS 目錄、搜索、閱讀與書架' };
}
