...
const sectionTabs = [
  { href: '/manga', label: '推薦', icon: Compass },
  { href: '/manga/search', label: '搜索', icon: Search },
  { href: '/manga/shelf', label: '書架', icon: BookOpen },
  { href: '/manga/history', label: '歷史', icon: History },
];

function getMeta(pathname: string, searchParams: ReturnType<typeof useSearchParams>) {
  if (pathname === '/manga/shelf') {
    return { title: '漫畫書架', subtitle: '集中管理收藏的漫畫' };
  }
  if (pathname === '/manga/history') {
    return { title: '漫畫歷史', subtitle: '從上次閱讀的位置繼續' };
  }
  if (pathname === '/manga/search') {
    return { title: '漫畫搜索', subtitle: '按標題和來源搜索漫畫' };
  }
  if (pathname === '/manga/detail') {
    return {
      title: searchParams.get('title') || '漫畫詳情',
      subtitle: searchParams.get('sourceName') || '漫畫詳情',
  ...
  if (pathname === '/manga/read') {
    ...
    return {
      title,
      subtitle: searchParams.get('chapterName') || '章節',
