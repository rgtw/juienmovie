...
export default function MangaHistoryCard({ item, inShelf, onToggleShelf, onDelete }: MangaHistoryCardProps) {
  ...
  const subtitle = useMemo(
    () => `${item.chapterName} · 第 ${item.pageIndex + 1}/${item.pageCount} 頁`,
    [item]
  );
      ...
      >
        ...
        >
          {item.cover ? (
            ...
            <div className='flex h-full items-center justify-center text-sm text-gray-400'>暫無封面</div>
          )}
        </div>
