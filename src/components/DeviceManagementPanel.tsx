          ...
          <div className='flex-1 overflow-y-auto p-6'>
            {devicesLoading ? (
              ...
              <div className='text-center py-8'>
                ...
                <p className='text-sm text-gray-500 dark:text-gray-400'>暫無登錄設備</p>
              </div>
            ) : (
              <div className='space-y-3'>
                {devices
                        ...
                        <div className='flex items-start justify-between'>
                          <div className='flex-1'>
                            ...
                            <div className='space-y-1 text-xs text-gray-500 dark:text-gray-400'>
                              <div>登錄時間: {new Date(device.createdAt).toLocaleString('zh-CN')}</div>
                              <div>最後活躍: {new Date(device.lastUsed).toLocaleString('zh-CN')}</div>
                            </div>
                          </div>
                          {!device.isCurrent && (
                            <button
                            ...
                            >
                              {revoking === device.tokenId ? '撤銷中...' : '撤銷'}
                            </button>
                          )}
                        </div>
                      </div>
