...
export default function AcgSearch({
  ...
  const [allItems, setAllItems] = useState<AcgSearchItem[]>([]); // 所有加載的項目
  ...
  const performSearch = async (page: number, isLoadMore = false) => {
    ...
    try {
      ...
      const errorMsg = err.message || '搜索失敗，請稍後重試';
      setError(errorMsg);
      onError?.(errorMsg);
    } finally {
      setLoading(false);
  ...
  // 使用 Intersection Observer 監聽滾動到底部
  useEffect(() => {
  ...
  const handleConfirmDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || '添加下載任務失敗');
      }

      setToast({
        message: data.message || '已添加到離線下載隊列',
        type: 'success',
        onClose: () => setToast(null),
      });
    } catch (err: any) {
      setToast({
        message: err.message || '添加下載任務失敗',
        type: 'error',
        onClose: () => setToast(null),
      });
    } finally {
