...
export default function ChatFloatingWindow() {
  ...
  const handleLeaveRoom = () => {
    if (confirm(isOwner ? '確定要解散房間嗎？所有成員將被踢出房間。' : '確定要退出房間嗎？')) {
      leaveRoom();
            ...
            <div className='space-y-4'>
              ...
              <div className='space-y-3'>
                <div className='flex items-center justify-between rounded-lg bg-gray-50 dark:bg-gray-800 p-4 border border-gray-200 dark:border-gray-700'>
                  <span className='text-sm font-medium text-gray-600 dark:text-gray-400'>房間名稱</span>
                  <span className='text-sm font-semibold text-gray-900 dark:text-gray-100'>{currentRoom.name}</span>
                </div>

                <div className='flex items-center justify-between rounded-lg bg-gray-50 dark:bg-gray-800 p-4 border border-gray-200 dark:border-gray-700'>
                  <span className='text-sm font-medium text-gray-600 dark:text-gray-400'>房間號</span>
                ...
                {currentRoom.description && (
                  <div className='rounded-lg bg-gray-50 dark:bg-gray-800 p-4 border border-gray-200 dark:border-gray-700'>
                    <span className='text-sm font-medium text-gray-600 dark:text-gray-400 block mb-2'>房間描述</span>
              ...
              <div className='rounded-lg bg-gray-50 dark:bg-gray-800 p-4 border border-gray-200 dark:border-gray-700'>
                <div className='flex items-center gap-2 mb-3'>
                  ...
                  <span className='text-sm font-medium text-gray-600 dark:text-gray-400'>成員列表 ({members.length})</span>
                </div>
    ...
    <>
        ...
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-white/80">{members.length} 人在線</p>
            </div>
          </div>
        ...
        <div className="border-t border-white/10 px-4 py-2">
          <div className="flex items-center justify-center gap-3 mb-1">
            ...
            >
              ...
              <span>{isMicEnabled ? '麥克風開' : '麥克風關'}</span>
            </button>
            ...
            >
              ...
              <span>{isSpeakerEnabled ? '喇叭開' : '喇叭關'}</span>
            </button>
          </div>

          {/* 狀態指示 */}
          <div className="text-center text-xs text-white/60">
            {voiceChat.isConnecting && '正在連接...'}
            {voiceChat.error && (
              <span className="text-red-300">{voiceChat.error}</span>
            )}
            {!voiceChat.isConnecting && !voiceChat.error && isMicEnabled && (
              <span>
                {voiceChat.strategy === 'webrtc-fallback' ? 'WebRTC模式' : '服務器中轉模式'}
              </span>
            )}
          </div>
        </div>
        ...
        {chatMessages.length === 0 ? (
          <div className="flex h-full items-center justify-center text-center">
            <div>
              ...
              <p className="text-sm text-gray-400">還沒有消息</p>
              <p className="text-xs text-gray-500">發送第一條消息吧</p>
            </div>
          </div>
        ) : (
          <>
      ...
      <div className="rounded-b-2xl bg-gray-900/50 px-4 py-2 text-center text-xs text-gray-400">
        {isOwner ? (
          ...
          <span>房間: {watchRoom.currentRoom.name}</span>
        )}
      </div>
    </div>
    </>
