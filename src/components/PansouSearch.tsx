...
export default function PansouSearch({
      ...
      const updates = await Promise.all(
        runningEntries.map(async ([cloudType, state]) => {
          try {
            ...
            if (!response.ok) {
              throw new Error(data.error || '獲取檢測進度失敗');
            }
            return {
              cloudType,
              taskId: state.taskId,
            ...
            return {
              ...
              task: {
                ...
                status: 'failed',
                error:
                  error instanceof Error ? error.message : '獲取檢測進度失敗',
  ...
  const searchPansou = useCallback(async () => {
    ...
    try {
      ...
      const errorMsg = err.message || '搜索失敗，請檢查配置';
      setError(errorMsg);
      onError?.(errorMsg);
    } finally {
      setLoading(false);
  ...
  const handleQuarkTransfer = async (link: PansouLink) => {
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || '轉存失敗');
      }

      setToast({
        message: `轉存成功，已保存到：${data.targetPath}`,
        type: 'success',
        onClose: () => setToast(null),
      });
    } catch (err: any) {
      setToast({
        message: err?.message || '轉存失敗',
        type: 'error',
        onClose: () => setToast(null),
      });
    } finally {
  ...
  const handleNetdiskInstantPlay = async (
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || '播放失敗');
      }
  ...
  const handleStartCheck = async (cloudType: string, links: PansouLink[]) => {
    try {
      ...
      if (!platform) {
        throw new Error('當前網盤類型暫不支持有效性檢測');
      }
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || '啟動檢測失敗');
      }
      setCheckStatesByType((prev) => ({
        ...prev,
        [cloudType]: {
  ...
  const handleCancelCheck = async (cloudType: string) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || '停止檢測失敗');
      }
      setCheckStatesByType((prev) => ({
        ...prev,
        [cloudType]: {
