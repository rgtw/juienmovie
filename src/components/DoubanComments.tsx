...
export default function DoubanComments({ doubanId }: DoubanCommentsProps) {
  ...
  const fetchComments = useCallback(async (startIndex: number) => {
    try {
      console.log('正在獲取評論，起始位置:', startIndex);
      setLoading(true);
      setError(null);
      ...
      if (startIndex === 0) {
        ...
        setComments((prev) => {
          console.log('追加評論，之前:', prev.length, '新增:', data.comments.length);
      ...
      console.log('更新後狀態 - hasMore:', data.hasMore, 'total:', data.total);
    } catch (err) {
      console.error('獲取評論失敗:', err);
      setError(err instanceof Error ? err.message : '獲取評論失敗');
    } finally {
      setLoading(false);
    }
  }, [doubanId]);
    ...
    return (
      <div className='flex flex-col items-center justify-center py-12'>
        <div className='text-gray-500 dark:text-gray-400 mb-4'>
          ...
          <p className='text-center'>點擊查看豆瓣評論</p>
        </div>
        <button
