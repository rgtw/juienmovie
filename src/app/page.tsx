...
function HomeClient() {
  ...
  const [aiDefaultMessageNoVideo, setAiDefaultMessageNoVideo] = useState('你好！我是MoonTVPlus的AI影視助手。想看什麼電影或劇集？需要推薦嗎？');
  ...
  const submitDirectPlay = async () => {
    ...
    try {
      ...
      if (netdisk) {
        ...
        if (!id) {
          throw new Error('網盤鏈接編碼失敗');
        }
        const targetUrl = `/play?source=${encodeURIComponent(source)}&id=${encodeURIComponent(id)}&title=${encodeURIComponent('網盤直鏈播放')}`;
      ...
      setToast({
        message: error instanceof Error ? error.message : '播放失敗',
        type: 'error',
      ...
      (async () => {
        try {
          ...
          try {
            ...
            console.error('獲取熱播短劇數據失敗:', error);
          }
