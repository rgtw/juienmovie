...
export default function WebLivePage() {
  ...
  function flvLoader(video: HTMLVideoElement, url: string) {
    ...
    flvPlayer.on(flvjs.Events.ERROR, (errorType: string, errorDetail: string) => {
      ...
      setErrorMessage(`播放失敗: ${errorType} - ${errorDetail}`);
      setVideoUrl('');
    });
    flvPlayer.load();
  ...
  const handleSourceClick = async (source: any) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setErrorMessage(data.error || '獲取直播流失敗');
      }
    } catch (err) {
      console.error('獲取直播流失敗:', err);
      setErrorMessage(err instanceof Error ? err.message : '獲取直播流失敗');
    } finally {
          ...
          <div className='text-center max-w-md mx-auto px-6'>
            ...
            <div className='mb-6 w-80 mx-auto'>
              <div className='flex justify-center space-x-2 mb-4'>
                <div
                  className={`w-3 h-3 rounded-full transition-all duration-500 ${loadingStage === 'loading' ? 'bg-primary-500 scale-125' : 'bg-primary-500'}`}
                ></div>
                <div
                  className={`w-3 h-3 rounded-full transition-all duration-500 ${loadingStage === 'fetching' ? 'bg-primary-500 scale-125' : 'bg-primary-500'}`}
                ></div>
                <div
                  className={`w-3 h-3 rounded-full transition-all duration-500 ${loadingStage === 'ready' ? 'bg-primary-500 scale-125' : 'bg-gray-300'}`}
                ></div>
              </div>
      ...
      <div className='flex flex-col gap-3 py-4 px-5 lg:px-[3rem] 2xl:px-20'>
        <div className='py-1'>
          <h1 className='text-xl font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2 max-w-[80%]'>
            <Radio className='w-5 h-5 text-primary-500 flex-shrink-0' />
            <div className='min-w-0 flex-1'>
              <div className='truncate'>
                {currentSource?.name || '網絡直播'}
              </div>
            </div>
          </h1>
        </div>
                ...
                {errorMessage && (
                  <div className='absolute inset-0 bg-black/90 backdrop-blur-sm rounded-xl overflow-hidden shadow-lg border border-white/0 dark:border-white/30 flex items-center justify-center z-[600] transition-all duration-300'>
                    <div className='text-center max-w-md mx-auto px-6'>
                      ...
                      <div className='space-y-4'>
                        <h3 className='text-xl font-semibold text-white'>獲取直播流失敗</h3>
                        <div className='bg-orange-500/20 border border-orange-500/30 rounded-lg p-4'>
                          <p className='text-orange-300 font-medium'>{errorMessage}</p>
                        </div>
                        <p className='text-sm text-gray-300'>請嘗試其他房間</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {isVideoLoading && (
                  <div className='absolute inset-0 bg-black/85 backdrop-blur-sm rounded-xl overflow-hidden shadow-lg border border-white/0 dark:border-white/30 flex items-center justify-center z-[500] transition-all duration-300'>
                    <div className='text-center max-w-md mx-auto px-6'>
                      ...
                      <div className='space-y-2'>
                        <p className='text-xl font-semibold text-white animate-pulse'>🔄 加載中...</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className='md:ml-2 px-4 py-0 h-full rounded-xl bg-black/10 dark:bg-white/5 flex flex-col border border-white/0 dark:border-white/30 overflow-hidden'>
                <div className='flex mb-1 -mx-6 flex-shrink-0'>
                  <div
                    ...
                    className={`flex-1 py-3 px-6 text-center cursor-pointer transition-all duration-200 font-medium ${activeTab === 'rooms' ? 'text-primary-600 dark:text-green-400' : 'text-gray-700 hover:text-primary-600 bg-black/5 dark:bg-white/5 dark:text-gray-300 dark:hover:text-green-400 hover:bg-black/3 dark:hover:bg-white/3'}`}
                  >
                    房间
                  </div>
                  <div
                    onClick={() => setActiveTab('platforms')}
                    className={`flex-1 py-3 px-6 text-center cursor-pointer transition-all duration-200 font-medium ${activeTab === 'platforms' ? 'text-primary-600 dark:text-green-400' : 'text-gray-700 hover:text-primary-600 bg-black/5 dark:bg-white/5 dark:text-gray-300 dark:hover:text-green-400 hover:bg-black/3 dark:hover:bg-white/3'}`}
                  >
                    平台
                  </div>
                </div>
                    ...
                    {selectedPlatform && (
                      <div className='mb-3 flex items-center justify-between px-2 py-2 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800'>
                        <div className='flex items-center gap-2'>
                          <span className='text-xs text-green-700 dark:text-green-300'>篩選平臺:</span>
                          <span className='text-sm font-medium text-green-800 dark:text-green-200'>
                            {selectedPlatform === 'huya' ? '虎牙' : selectedPlatform === 'bilibili' ? '嗶哩嗶哩' : selectedPlatform === 'douyin' ? '抖音' : selectedPlatform}
                          </span>
                        </div>
                        <button
                    ...
                    {filteredSources.length > 0 ? (
                          ...
                          >
                            <div className='flex items-center gap-3'>
                              ...
                              <div className='flex-1 min-w-0'>
                                ...
                                <div className='text-xs text-gray-500 dark:text-gray-400 mt-1'>房間ID: {source.roomId}</div>
                              </div>
                            </div>
                          </button>
                        );
                      ...
                      <div className='flex flex-col items-center justify-center py-12 text-center'>
                        ...
                        <p className='text-gray-500 dark:text-gray-400 font-medium'>
                          {selectedPlatform ? '該平臺暫無可用房間' : '暫無可用房間'}
                        </p>
                      </div>
                    )}
                  </div>
                    ...
                    <div className='flex-1 overflow-y-auto space-y-2 pb-20'>
                      {platforms.length > 0 ? (
                          ...
                          >
                            <div className='w-12 h-12 bg-gray-200 dark:bg-gray-600 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden'>
                              {platform === 'huya' ? (
                                ...
                                <img src='https://www.bilibili.com/favicon.ico' alt='嗶哩嗶哩' className='w-8 h-8' />
                              ) : platform === 'douyin' ? (
                        ...
                        <div className='flex flex-col items-center justify-center py-12 text-center'>
                          ...
                          <p className='text-gray-500 dark:text-gray-400 font-medium'>暫無可用平臺</p>
                        </div>
                      )}
                    </div>
                  </div>
