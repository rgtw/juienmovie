  ...
  return (
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        <div className='text-sm text-gray-500'>共 {items.length} 條閱讀歷史</div>
        <button
          type='button'
          ...
          <div key={item.storageKey} className='rounded-3xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-950'>
            <div className='flex gap-4'>
              ...
              <div className='min-w-0 flex-1'>
                ...
                <div className='mt-1 text-xs text-gray-500'>已讀 {Math.round(item.progressPercent || 0)}% · {getReadableChapterLabel(item)}</div>
                <div className='mt-3 flex flex-wrap gap-2'>
                  ...
                  <button onClick={async () => { const [deleteSourceId = item.sourceId, deleteBookId = item.bookId] = item.storageKey.split('+'); await deleteBookReadRecord(deleteSourceId, deleteBookId); updateRecords((() => { const next = { ...records }; delete next[item.storageKey]; return next; })()); }} className='rounded-2xl border border-gray-200 px-3 py-2 text-xs dark:border-gray-700'>刪除</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {!loading && items.length === 0 ? <div className='text-sm text-gray-500'>暫無閱讀歷史</div> : null}
          ...
          <div className='absolute right-0 top-0 h-screen w-full max-w-lg overflow-y-auto bg-white shadow-2xl dark:bg-gray-950' onClick={(event) => event.stopPropagation()}>
            <div className='space-y-4 p-5'>
              <div className='flex items-start justify-between gap-4'>
                <div>
                  <div className='text-base font-semibold'>緩存管理</div>
                  <div className='mt-1 text-xs text-gray-500'>已緩存 {cacheItems.length} 本 · {formatBytes(cacheTotalSize)}</div>
                </div>
                <div className='flex gap-2'>
                  <button type='button' onClick={() => void loadCacheItems()} className='inline-flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 dark:border-gray-700' aria-label='刷新緩存' title='刷新緩存'><RefreshCw className='h-4 w-4' /></button>
                  <button type='button' onClick={() => setConfirmAction({ type: 'clear-all' })} className='inline-flex h-9 w-9 items-center justify-center rounded-full border border-red-200 text-red-600 dark:border-red-900/60 dark:text-red-400' aria-label='清空全部緩存' title='清空全部緩存'><Trash2 className='h-4 w-4' /></button>
                  <button type='button' onClick={() => setCacheModalOpen(false)} className='inline-flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 dark:border-gray-700' aria-label='關閉' title='關閉'><X className='h-4 w-4' /></button>
                </div>
              </div>

              {cacheLoading ? <div className='text-sm text-gray-500'>正在讀取緩存…</div> : null}
              {!cacheLoading && cacheItems.length === 0 ? <div className='text-sm text-gray-500'>當前還沒有緩存書籍</div> : null}
                  ...
                  <div key={item.key} className='rounded-3xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900'>
                    <div className='flex items-start justify-between gap-3'>
                      <div className='min-w-0 flex-1'>
                        ...
                        <div className='mt-1 text-xs text-gray-500'>最近打開 {new Date(item.lastOpenTime).toLocaleString()}</div>
                      </div>
                      <button
                        type='button'
        ...
        <div className='fixed inset-0 z-[60] flex items-center justify-center bg-black/50 px-4' onClick={() => setConfirmAction(null)}>
          <div className='w-full max-w-sm rounded-3xl border border-gray-200 bg-white p-5 shadow-2xl dark:border-gray-700 dark:bg-gray-950' onClick={(event) => event.stopPropagation()}>
            <div className='text-base font-semibold text-gray-900 dark:text-gray-100'>
              {confirmAction.type === 'clear-all' ? '清空全部緩存' : '刪除緩存'}
            </div>
            <div className='mt-2 text-sm text-gray-500 dark:text-gray-400'>
              {confirmAction.type === 'clear-all'
                ? '確認清空當前瀏覽器中的全部電子書緩存嗎？此操作不可撤銷。'
                : `確認刪除《${confirmAction.title || '該書'}》的本地緩存嗎？`}
            </div>
