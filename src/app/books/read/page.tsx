...
async function loadEpubScript() {
  ...
  if (!window.JSZip) {
    await loadScriptOnce('script[data-jszip]', 'https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js', 'JSZip 加載失敗');
  }
  if (!window.ePub) {
    await loadScriptOnce('script[data-epubjs]', 'https://cdn.jsdelivr.net/npm/epubjs/dist/epub.min.js', 'epub.js 加載失敗');
  }
}
...
async function downloadBookWithProgress(
  ...
  if (!response.ok) throw new Error(`下載電子書失敗: ${response.status}`);
...
export default function BookReadPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/books/read/manifest', {
      ...
      .then(async (res) => {
        ...
        if (!res.ok) throw new Error(json.error || '獲取閱讀信息失敗');
        setManifest(json);
        cacheBookDetail(json.book);
      })
      .catch((err) => setError(err.message || '獲取閱讀信息失敗'));
  ...
  useEffect(() => {
    ...
    fetch('/api/books/tts/voices')
      .then(async (res) => {
        const json = await res.json();
        if (!res.ok) throw new Error(json.error || '獲取朗讀配置失敗');
        if (cancelled) return;
      ...
      .catch((err) => {
        ...
        setTtsVoices([]);
        setTtsError(err.message || '朗讀能力不可用');
      });
    return () => {
      cancelled = true;
    };
  ...
  const fetchTtsChunkAudioUrl = useCallback(async (chunk: TtsChunk, chapterHref: string) => {
    ...
    if (!manifest) throw new Error('書籍信息未準備好');
    ...
    if (!response.ok) throw new Error(json.error || '朗讀音頻生成失敗');
  ...
  const playTtsChunk = useCallback(async (index: number) => {
    ...
    try {
      ...
      setTtsStatus('error');
      setTtsLoadingChunkIndex(null);
      setTtsError((error as Error).message || '朗讀失敗');
    }
  ...
  const bootstrapTtsForCurrentChapter = useCallback(async (resume = true) => {
    ...
    if (!chapterHref) {
      setTtsError('當前章節尚未定位，稍後再試');
      setTtsStatus('error');
      return;
    }
    const text = getCurrentSpineDocumentText();
    if (!text) {
      setTtsError('當前章節暫未提取到可朗讀文本');
      setTtsStatus('error');
      return;
    }
    cleanupTtsAudioUrls();
  ...
  const toggleTtsPlayback = useCallback(async () => {
    ...
    if (ttsStatus === 'paused' && audioRef.current) {
      try {
        ...
        setTtsStatus('error');
        setTtsError((error as Error).message || '恢復播放失敗');
      }
      return;
    }
  ...
  useEffect(() => {
    ...
    loadEpubScript()
      .then(async () => {
        ...
        rendition.on('relocated', (location: EpubLocation) => {
          ...
          if (restoreTarget && !restoreMessageShown) {
            ...
            setRestoredMessage(`已恢復到上次閱讀位置（約 ${Math.round(manifest.lastRecord?.progressPercent || 0)}%）`);
      ...
      .catch((err) => {
        setReady(false);
        setError(err.message || '初始化 EPUB 閱讀器失敗');
      });
  ...
  useEffect(() => {
    ...
    downloadBookWithProgress(manifest, (received, total) => {
      ...
      .catch((err) => {
        if (!cancelled) setError(err.message || 'PDF 加載失敗');
      });
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent('books-read-update-header', {
      detail: {
        title: manifest.book.title,
        subtitle: currentTocLabel || currentChapter || manifest.book.author || (settings.mode === 'scrolled' ? '滾動閱讀' : '分頁閱讀'),
    ...
    return (
      <div className='flex h-[calc(100vh-3.5rem)] items-center justify-center bg-white px-4 dark:bg-gray-950'>
        <div className='flex flex-col items-center gap-4 text-center'>
          ...
          <div className='text-sm text-gray-500 dark:text-gray-400'>準備閱讀器中...</div>
        </div>
      </div>
    );
  }
  ...
  if (manifest.format === 'pdf') {
    if (!pdfBlobUrl) return <div className='p-4 text-sm text-gray-500'>PDF 加載中... {progressLabel}</div>;
    ...
    <div className='flex h-[calc(100vh-3.5rem)] flex-col bg-white dark:bg-gray-950'>
          ...
          <div className='mx-auto max-w-3xl space-y-4'>
            <div className='space-y-2 rounded-3xl border border-gray-200 bg-white/90 p-5 shadow-sm dark:border-gray-800 dark:bg-gray-950/90'>
              ...
              <div className='flex items-center justify-between text-xs text-gray-500 dark:text-gray-400'>
                <span>{cacheHit ? '已命中本地緩存' : '首次打開將緩存到當前瀏覽器'}</span>
            ...
            <div className='rounded-3xl bg-gray-50 p-6 dark:bg-gray-900'>
              {fileLoadState === 'opening' ? (
                <div className='flex min-h-32 flex-col items-center justify-center gap-4 text-center'>
                  ...
                  <div className='text-sm text-gray-500 dark:text-gray-400'>正在打開電子書...</div>
                </div>
              ) : (
          ...
          >
            <div className='p-4'>
              <div className='space-y-2' ref={tocScrollRef}>
                {tocItems.length === 0 ? (
                  <div className='p-3 text-sm text-gray-500'>當前 EPUB 未提供目錄</div>
                ) : (
      ...
      {settingsOpen && typeof document !== 'undefined' ? createPortal(
        <div className='fixed inset-0 z-40 flex items-center justify-center bg-black/40 px-4' onClick={() => setSettingsOpen(false)}>
          ...
          >
            <div className='mb-4'>
              <div className='text-base font-semibold text-gray-900 dark:text-gray-100'>閱讀設置</div>
              <div className='mt-1 text-xs text-gray-500'>可切換翻頁或滾動閱讀，默認翻頁模式</div>
            </div>
            <div className='space-y-6 p-1 text-sm'>
              <div>
                <div className='mb-2 font-medium'>閱讀模式</div>
                <div className='grid grid-cols-2 gap-2'>
                  {([
                    { key: 'paginated', label: '翻頁模式', desc: '左右點擊翻頁' },
                    { key: 'scrolled', label: '滾動模式', desc: '上下連續滾動' },
              ...
              <div>
                <div className='mb-2 font-medium'>主題</div>
                <div className='grid grid-cols-3 gap-2'>
                  {(['light', 'sepia', 'dark'] as ReaderTheme[]).map((theme) => (
                    ...
                    >
                      ...
                      {theme === 'light' ? '淺色' : theme === 'sepia' ? '護眼' : '深色'}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <div className='mb-2 flex items-center justify-between font-medium'>字號 <span>{settings.fontSize}%</span></div>
            ...
            <div className='overflow-hidden rounded-3xl border border-gray-200 bg-white/95 shadow-xl backdrop-blur dark:border-gray-800 dark:bg-gray-950/95'>
              ...
              <div className='px-3 py-2.5'>
                <div className='flex items-center gap-2'>
                  ...
                  <div className='min-w-0 flex-1'>
                    <div className='truncate text-sm font-medium text-gray-900 dark:text-gray-100'>
                      {ttsCurrentChapterTitle || currentTocLabel || currentChapter || '語音朗讀'}
                    </div>
                ...
                <div className='mt-2 flex items-center justify-between text-[11px] text-gray-400'>
                  <span>{selectedVoice?.displayName || '默認音色'}</span>
            ...
            <div className='absolute inset-x-0 bottom-20 z-30 mx-auto w-[min(94vw,34rem)]'>
              <div className='rounded-[2rem] border border-gray-200 bg-white/98 p-4 shadow-2xl backdrop-blur dark:border-gray-800 dark:bg-gray-950/98'>
                ...
                <div className='mb-4 flex items-center justify-between rounded-2xl bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:bg-gray-900 dark:text-gray-300'>
                  <span className='truncate'>{currentChunk?.text.slice(0, 28) || '當前章節可開始朗讀'}</span>
                ...
                <div className='space-y-4'>
                  ...
                  <label className='block'>
                    <div className='mb-2 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400'>
                      <span className='flex items-center gap-2'><Gauge className='h-3.5 w-3.5' />語速</span>
                  ...
                  <label className='block'>
                    <div className='mb-2 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400'>
                      <span className='flex items-center gap-2'><Waves className='h-3.5 w-3.5' />音調</span>
