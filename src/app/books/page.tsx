...
export default function BooksHomePage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/books/sources')
      ...
      .catch((err) => setError(err.message || '加載書源失敗'))
  ...
  return (
    <div className='space-y-6'>
      <section className='rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-950'>
        <h1 className='text-lg font-semibold'>OPDS 電子書源</h1>
        <p className='mt-1 text-sm text-gray-500 dark:text-gray-400'>支持分類瀏覽、搜索、書架與 EPUB 在線閱讀。</p>
      </section>
        ...
        {sources.map((source) => (
          <div key={source.id} className='rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-950'>
            ...
            <div className='mt-2 flex flex-wrap gap-2 text-xs'>
              <span className={`rounded-full px-2 py-1 ${source.capabilities?.catalogSupported ? 'bg-sky-100 text-sky-700 dark:bg-sky-950/50 dark:text-sky-300' : 'bg-gray-100 text-gray-500 dark:bg-gray-900 dark:text-gray-400'}`}>分類{source.capabilities?.catalogSupported ? '可用' : '不可用'}</span>
            ...
            <div className='mt-4 flex flex-wrap gap-2'>
              {source.capabilities?.catalogSupported && <Link href={`/books/catalog?sourceId=${encodeURIComponent(source.id)}`} className='rounded-2xl bg-sky-600 px-4 py-2 text-sm text-white'>瀏覽目錄</Link>}
              {source.capabilities?.searchSupported && <Link href={`/books/search?sourceId=${encodeURIComponent(source.id)}`} className='rounded-2xl border border-gray-200 px-4 py-2 text-sm dark:border-gray-700'>搜索書籍</Link>}
            </div>
          </div>
        ))}
      </div>
