  ...
  useEffect(() => {
    ...
    fetch('/api/books/detail', {
      ...
      .then(async (res) => {
        ...
        if (!res.ok) throw new Error(json.error || '獲取詳情失敗');
        setDetail(json);
        cacheBookDetail(json);
      })
      .catch((err) => setError(err.message || '獲取詳情失敗'));
    ...
    <div className='space-y-6'>
      <section className='grid gap-6 rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-950 md:grid-cols-[220px_1fr]'>
        <div className='overflow-hidden rounded-3xl bg-gray-100 dark:bg-gray-900'>
          {detail.cover ? <img src={detail.cover} alt={detail.title} className='h-full w-full object-cover' /> : <div className='flex aspect-[3/4] items-center justify-center text-sm text-gray-400'>無封面</div>}
        </div>
        <div className='space-y-4'>
          <div>
          ...
          <div className='flex flex-wrap gap-3'>
            {readable ? <Link href={buildBookReadPath(detail.sourceId, detail.id)} onClick={() => cacheBookDetail(detail)} className='rounded-2xl bg-sky-600 px-4 py-2 text-sm text-white'>在線閱讀</Link> : null}
            <button onClick={toggleShelf} className='rounded-2xl border border-gray-200 px-4 py-2 text-sm dark:border-gray-700'>{shelf[`${detail.sourceId}+${detail.id}`] ? '移出書架' : '加入書架'}</button>
            {readable ? <button onClick={async () => { try { setFileBusy('download'); await openBookFile(detail.sourceId, detail.id, readableFormat, true, readable?.href, detail.title); } catch (err) { setError((err as Error).message || '下載文件失敗'); } finally { setFileBusy(''); } }} disabled={fileBusy !== ''} className='rounded-2xl border border-gray-200 px-4 py-2 text-sm dark:border-gray-700'>{fileBusy === 'download' ? '下載中...' : '下載文件'}</button> : null}
          </div>
        </div>
      </section>
              ...
              <div key={`${item.href}-${item.type}`} className='flex items-center justify-between rounded-2xl bg-gray-50 px-4 py-3 text-sm dark:bg-gray-900'>
                ...
                <button disabled={!format || fileBusy !== ''} onClick={async () => {
                  ...
                  try {
                    ...
                    setError((err as Error).message || '打開文件失敗');
                  } finally {
                    setFileBusy('');
                  }
                }} className='text-sky-600 disabled:text-gray-400'>打開</button>
              </div>
            );
          })}
        </div>
