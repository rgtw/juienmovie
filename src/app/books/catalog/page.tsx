...
export default function BooksCatalogPage() {
  ...
  const loadCatalog = useCallback(async (targetHref?: string, append = false) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error || '獲取目錄失敗');
      ...
      setError(err instanceof Error ? err.message : '獲取目錄失敗');
    } finally {
        ...
        <>
          ...
          {navigationItems.length > 0 ? (
            <section className='space-y-3'>
              <div className='text-sm font-medium text-gray-700 dark:text-gray-300'>目錄</div>
              <div
                ref={navScrollerRef}
                ...
                {navigationItems.map((item, index) => (
                  <Link
                   ...
                   >
                    ...
                    <div className='mt-2 text-xs text-gray-500 dark:text-gray-400'>點擊進入子目錄</div>
                  </Link>
                ))}
              </div>
            </section>
