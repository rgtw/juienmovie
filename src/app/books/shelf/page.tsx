...
export default function BookShelfPage() {
  ...
  return (
    <div className='space-y-4'>
      <div className='text-sm text-gray-500'>共 {items.length} 本電子書</div>
          ...
          <div key={`${item.sourceId}-${item.bookId}`} className='rounded-3xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-950'>
            <div className='flex gap-4'>
              ...
              <div className='min-w-0 flex-1'>
                ...
                <div className='mt-2 text-xs text-gray-500'>進度 {Math.round(item.progressPercent || 0)}%</div>
                <div className='mt-3 flex flex-wrap gap-2'>
                  <Link href={buildBookDetailPath(item.sourceId, item.bookId)} onClick={() => cacheBookShelfItem(item)} className='rounded-2xl bg-sky-600 px-3 py-2 text-xs text-white'>詳情</Link>
      ...
      {items.length === 0 ? <div className='text-sm text-gray-500'>書架還是空的</div> : null}
    </div>
  );
}
