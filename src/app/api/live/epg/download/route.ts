...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!epgUrl) {
      return NextResponse.json({ error: '缺少EPG URL參數' }, { status: 400 });
    }
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'EPG文件下載失敗' }, { status: 500 });
    }
    ...
    if (isGzip) {
      ...
      if (!reader) {
        return NextResponse.json({ error: '無法讀取響應' }, { status: 500 });
      }
