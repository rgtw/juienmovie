...
export async function GET(request: NextRequest) {
  ...
  try {
    // 注意：records 按“播放隊列順序”返回（createdAt ASC），
    // 前端再基於 lastPlayedAt 定位當前播放項。
    const records = await db.listMusicV2History(username);
    return NextResponse.json({ success: true, data: { records } });
  } catch (error) {
    return internalError('獲取播放歷史失敗', (error as Error).message);
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!record.songId || !record.source || !record.name || !record.artist) {
      return badRequest('歷史記錄數據不完整');
    }

    await db.upsertMusicV2History(username, record);
    return NextResponse.json({ success: true, data: { record } });
  } catch (error) {
    return internalError('保存播放歷史失敗', (error as Error).message);
  }
}
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    return internalError('刪除播放歷史失敗', (error as Error).message);
  }
}
