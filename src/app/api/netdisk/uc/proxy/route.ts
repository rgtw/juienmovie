...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!selected) {
      return NextResponse.json({ error: '未獲取到 UC 播放地址' }, { status: 500 });
    }
    ...
    try {
      ...
      if (!upstream.ok || !upstream.body) {
        return NextResponse.json(
          { error: `UC視頻代理失敗 (${upstream.status})` },
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        return NextResponse.json({ error: 'UC網盤代理超時' }, { status: 504 });
      }
      throw error;
    }
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'UC網盤代理失敗' },
      { status: 500 }
    );
  }
}
