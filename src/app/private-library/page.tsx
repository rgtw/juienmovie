...
export default function PrivateLibraryPage() {
  ...
  useEffect(() => {
    const fetchEmbySources = async () => {
      try {
        ...
        console.error('獲取Emby源列表失敗:', error);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchEmbyViews = async () => {
      ...
      try {
        ...
        if (data.error) {
          console.error('獲取 Emby 媒體庫列表失敗:', data.error);
          setEmbyViews([]);
        } else {
        ...
        console.error('獲取 Emby 媒體庫列表失敗:', err);
        setEmbyViews([]);
      } finally {
  ...
  const sortOptions = [
    ...
    { label: '發行日期', value: 'PremiereDate' },
    { label: '年份', value: 'ProductionYear' },
    { label: '評分', value: 'CommunityRating' },
  ];
          ...
          <div className='space-y-4'>
            ...
            {xiaoyaFolders.length === 0 && xiaoyaFiles.length === 0 && (
              <div className='text-center py-12'>
                <p className='text-gray-500 dark:text-gray-400'>此目錄為空</p>
              </div>
            )}
              </>
            )}
