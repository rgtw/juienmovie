...
const READ_MODE_OPTIONS: Array<{ value: ReadMode; label: string }> = [
  ...
  { value: 'vertical', label: '垂直滾動' },
  { value: 'horizontal', label: '水平滾動' },
];

const SCALE_MODE_OPTIONS: Array<{ value: ScaleMode; label: string }> = [
  { value: 'fit', label: '適配屏幕' },
...
export default function MangaReadPage() {
  ...
  const title = searchParams.get('title') || '漫畫閱讀';
  const cover = searchParams.get('cover') || '';
  const sourceName = searchParams.get('sourceName') || sourceId;
  const chapterName = searchParams.get('chapterName') || '章節';
        ...
        >
          ...
          >
            <div className='mb-4'>
              ...
              <div className='mt-1 text-xs text-gray-500'>可繼續擴展更多閱讀參數</div>
            </div>

            <div className='space-y-5'>
              <div>
                <div className='mb-2 text-sm font-medium text-gray-700 dark:text-gray-200'>顯示方式</div>
              ...
              <div>
                <div className='mb-2 text-sm font-medium text-gray-700 dark:text-gray-200'>縮放類型</div>
                <div className='grid grid-cols-2 gap-2'>
              ...
              <div>
                ...
                />
                <div className='mt-1 text-xs text-gray-500'>滾動閱讀時，兩張圖片之間的間隔</div>
              </div>
        ...
        {showChapterComplete && (
          <div className='fixed inset-0 z-30 flex items-center justify-center bg-black/60 px-4' onClick={() => setShowChapterComplete(false)}>
            <div
              ...
              onClick={(event) => event.stopPropagation()}
            >
              <div className='text-lg font-semibold text-gray-900 dark:text-gray-100'>{chapterName} 閱讀完畢</div>
              <div className='mt-2 text-sm text-gray-500 dark:text-gray-400'>
                {nextChapter ? '當前章節已讀完，可繼續閱讀下一話' : '當前章節已讀完'}
              </div>
