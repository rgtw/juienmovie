...
export default function WatchRoomScreenPage() {
  ...
  useEffect(() => {
    ...
    if (supportError) {
      showToast(`當前設備無法使用屏幕共享房間：${supportError}`, 'error');
      leaveRoom();
      router.replace('/watch-room');
    }
  ...
  const captureSettingsText = captureSettings
    ? [
        captureSettings.width && captureSettings.height
          ...
          : '分辨率未知',
        captureSettings.frameRate ? `${Math.round(captureSettings.frameRate)} fps` : '幀率未知',
      ].join(' / ')
    : '未開始';
        ...
        <div className='grid flex-1 grid-cols-1 gap-4 xl:grid-cols-[1fr_320px]'>
            ...
            {!isSharing && (
              <div className='absolute px-6 text-center text-white'>
                ...
                <p className='text-lg font-medium'>
                  {isOwner ? '點擊開始共享，向房員推送瀏覽器畫面' : '等待房主開始共享屏幕'}
                </p>
                {isOwner && (
          ...
          <div className='space-y-4'>
            <div className='rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900'>
              <h2 className='mb-3 font-semibold'>共享狀態</h2>
              <div className='space-y-2 text-sm text-gray-600 dark:text-gray-400'>
                <p>類型：屏幕共享</p>
                <p>狀態：{isSharing ? '共享中' : '未開始'}</p>
                <p>成員：{members.length} 人</p>
              </div>
