...
function getScreenShareHostSupportError() {
  ...
  if (!window.isSecureContext) {
    return '當前環境不是安全上下文（HTTPS/localhost），不支持屏幕共享';
  }
...
export default function WatchRoomPage() {
  ...
  const handleCreateRoom = async (e: React.FormEvent) => {
    ...
    if (createForm.roomType === 'screen') {
      ...
      if (supportError) {
        showToast(`當前設備無法創建屏幕共享房間：${supportError}`, 'error');
        return;
      }
    }
    ...
    try {
      ...
      showToast(error.message || '創建房間失敗', 'error');
    } finally {
  ...
  const handleJoinRoom = async (e: React.FormEvent, roomId?: string) => {
    ...
    if (targetRoom?.roomType === 'screen') {
      ...
      if (supportError) {
        showToast(`當前設備無法加入屏幕共享房間：${supportError}`, 'error');
        return;
      }
    }
  ...
  const tabs = [
    { id: 'create' as TabType, label: '創建房間', icon: Users },
    { id: 'join' as TabType, label: '加入房間', icon: UserPlus },
    { id: 'list' as TabType, label: '房間列表', icon: ListIcon },
  ];
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-bold mb-1">
                    {currentRoom.roomType === 'screen'
                      ? currentRoom.currentState?.type === 'screen' ? '房主正在共享屏幕' : '等待房主開始共享'
                  ...
                  {currentRoom.currentState && (
                    <p className="text-xs text-white/90 mt-1">
                      {currentRoom.currentState.type === 'play'
                        ? `${currentRoom.currentState.videoName || '未知視頻'}`
                        : currentRoom.currentState.type === 'live'
                          ? `${currentRoom.currentState.channelName || '未知頻道'}`
                          : '屏幕共享進行中'}
                    </p>
                  )}
                  {!currentRoom.currentState && (
                    <p className="text-xs text-white/70 mt-1">
                      {currentRoom.roomType === 'screen' ? '當房主開始共享時，您將自動進入共享頁' : '當房主開始播放時，您將自動跟隨'}
                    </p>
                  )}
                </div>
              </div>
                ...
                {currentRoom ? (
                  <div className="space-y-4">
                    ...
                    <div className="bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl p-6 text-white">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          ...
                          <p className="text-blue-100 text-sm">{currentRoom.description || '暫無描述'}</p>
                        </div>
                        {isOwner && (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                        <div className="bg-white/10 backdrop-blur rounded-lg p-3">
                          <p className="text-blue-100 text-xs mb-1">房間號</p>
                          <p className="text-xl font-mono font-bold">{currentRoom.id}</p>
                        </div>
                        <div className="bg-white/10 backdrop-blur rounded-lg p-3">
                          <p className="text-blue-100 text-xs mb-1">成員數</p>
                          <p className="text-xl font-bold">{members.length} 人</p>
                        </div>
                        <div className="bg-white/10 backdrop-blur rounded-lg p-3">
                          <p className="text-blue-100 text-xs mb-1">房間類型</p>
                          <p className="text-base font-bold">{currentRoom.roomType === 'screen' ? '屏幕共享' : '進度同步'}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
                      <p className="text-sm text-blue-800 dark:text-blue-200">
                        💡 {currentRoom.roomType === 'screen'
                          ? '這是屏幕共享房間，創建後將進入共享頁，由房主發起屏幕共享'
                          : '前往播放頁面或直播頁面開始觀影，房間成員將自動同步您的操作'}
                      </p>
                    </div>
                  </div>
                ) : (
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      >
                        <div className="font-medium text-gray-900 dark:text-gray-100">進度同步</div>
                        <div className="mt-1 text-sm text-gray-600 dark:text-gray-400">統一播放進度（適合雙方網絡穩定的情況）</div>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <div className="mt-1 text-sm text-gray-600 dark:text-gray-400">房員直接觀看房主共享的瀏覽器畫面（適合完全實時同步的情況）</div>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-gradient-to-r from-green-500 to-teal-600 rounded-xl p-6 text-white">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="text-2xl font-bold mb-1">{currentRoom.name}</h3>
                          <p className="text-green-100 text-sm">{currentRoom.description || '暫無描述'}</p>
                        </div>
                        {isOwner && (
                          <span className="bg-yellow-400 text-yellow-900 px-3 py-1 rounded-full text-xs font-bold">
                            房主
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                        <div className="bg-white/10 backdrop-blur rounded-lg p-3">
                          <p className="text-green-100 text-xs mb-1">房間號</p>
                          <p className="text-xl font-mono font-bold">{currentRoom.id}</p>
                        </div>
                        <div className="bg-white/10 backdrop-blur rounded-lg p-3">
                          <p className="text-green-100 text-xs mb-1">成員數</p>
                          <p className="text-xl font-bold">{members.length} 人</p>
                        </div>
                        <div className="bg-white/10 backdrop-blur rounded-lg p-3">
                          <p className="text-green-100 text-xs mb-1">房間類型</p>
                          <p className="text-base font-bold">{currentRoom.roomType === 'screen' ? '屏幕共享' : '進度同步'}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-4 border border-green-200 dark:border-green-800">
                      <p className="text-sm text-green-800 dark:text-green-200">
                        💡 {currentRoom.roomType === 'screen'
                          ? '這是屏幕共享房間，進入後即可觀看房主共享畫面'
                          : isOwner ? '前往播放頁面或直播頁面開始觀影，房間成員將自動同步您的操作' : '等待房主開始播放，您的播放進度將自動跟隨房主'}
                      </p>
                    </div>
                  </div>
                ) : (
              ...
              {loading && rooms.length === 0 && (
                <div className="flex items-center justify-center py-20">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">加載中...</p>
                  </div>
                </div>
              )}
                    ...
                    >
                      ...
                      <div className="space-y-2 text-sm mb-4">
                        <div className="flex items-center justify-between">
                          <span className="text-gray-500 dark:text-gray-400">房間號</span>
                        ...
                        <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                          ...
                          <span>{room.memberCount} 人在線</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between text-gray-600 dark:text-gray-400">
                          <span>類型</span>
                          <span>{room.roomType === 'screen' ? '屏幕共享' : '進度同步'}</span>
                        </div>
                        <div className="flex items-center justify-between text-gray-600 dark:text-gray-400">
                          <span>創建時間</span>
                            ...
                            <p className="text-xs text-blue-700 dark:text-blue-300 truncate">
                              {room.currentState.type === 'play'
                                ...
                                : room.currentState.type === 'live'
                                  ? `正在觀看: ${room.currentState.channelName}`
                                  : '正在共享屏幕'}
                            </p>
                          </div>
                        )}
