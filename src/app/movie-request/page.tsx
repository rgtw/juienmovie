...
export default function MovieRequestPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setAlertModal({ isOpen: true, type: 'error', title: '搜索失敗', message: '請稍後重試' });
    } finally {
  ...
  const submitRequest = async (item: TMDBResult, season?: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAlertModal({ isOpen: true, type: 'error', title: '求片失敗', message: data.error || '請稍後重試' });
      }
    } catch (err) {
      console.error('求片失敗:', err);
      setAlertModal({ isOpen: true, type: 'error', title: '求片失敗', message: '請稍後重試' });
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className='container mx-auto px-4 py-6'>
        <div className='mb-6'>
          ...
          <p className='text-sm text-gray-500 dark:text-gray-400 mt-1'>
            {isFeatureEnabled ? '搜索並提交您想看的影片' : '求片功能已關閉，僅可查看已求片列表'}
          </p>
        </div>
                  ...
                  >
                    ...
                    <div className='p-3'>
                      ...
                      <div className={`text-xs px-2 py-1 rounded text-center ${
                        ...
                        {request.status === 'fulfilled' ? '已上架' : '待處理'}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {searchResults.map((item) => (
              ...
              >
                ...
                <div className='p-3'>
                  ...
                  >
                    {submitting ? '處理中...' : !isFeatureEnabled ? '功能已關閉' : '求片'}
                  </button>
                </div>
              </div>
            ))}
